...
const SectionCard = ({ indicator, title, editAction, fields, button }: SectionCardProps) => {
    ...
    return (
        <div>
            <div className="hotspot-custom-grid pt-[20px] lg:pt-[30px] relative">
                ...
                <div className="px-[20px] xl:px-[30px]">
                    <p className="text-[16px] lg:text-[20px] font-medium text-center">
                        {title} {editAction && <button onClick={editAction}>
                    ...
                    {fields.map((field, index) => <div key={index} className="flex items-center justify-between grid-item-text py-[14px]">
                        <p className="text-[12px] lg:text-[14px] text-lightest-white font-normal">
                            {field.name}
                        </p>
                        <p className={`text-[14px] lg:text-[16px] font-medium capitalize${field.colored && " gradient-text"}`}>
                            {field.value}
                        </p>
                    </div>)}
                </div>
